...
export const checkAvailability = query({
    args: { eventId: v.id("events") },
    handler: async (ctx, { eventId }): Promise<{ available: boolean; availableSpots: number; totalTickets: number; purchasedCount: number; activeOffers: number }> => {
...
export const joinWaitingList = mutation({
    ...
    handler: async (ctx, { eventId, userId }): Promise<{ success: boolean; status: string; message: string; }> => {
        // Rate limit check
